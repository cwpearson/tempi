  ...
  */
  ...
  if (reorder && Placement::RANDOM == environment::placement) {
    ...
    MPI_Gather(edgeDst.data(), edgeDst.size(), MPI_INT, edgeDst.data(),
               graphSize, MPI_INT, 0, *comm_dist_graph);
    MPI_Gather(weight.data(), weight.size(), MPI_INT, weight.data(), graphSize,
               MPI_INT, 0, *comm_dist_graph);
    ...
    if (0 == graphRank) {
      ...
      int metisErr = METIS_PartGraphKway(
          &nvtxs, &ncon, xadj.data(), adjncy.data(), vwgt, vsize, adjwgt.data(),
