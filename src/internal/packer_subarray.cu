...
pack_bytes(void *__restrict__ outbuf, int position, // position in output buffer
           const void *__restrict__ inbuf, const int incount,
           unsigned ndims,       // number of subarray dimensions
           unsigned blockLength, // block length (B)
           unsigned *starts,     // start of dim i (B)
           unsigned *strides,    // stride of dim i (B)
           unsigned *extents) {

  assert(blockLength % N == 0); // N should evenly divide block length

  // distribute grid across bottom 3 dimensions
  const unsigned int tz = blockDim.z * blockIdx.z + threadIdx.z;
  const unsigned int ty = blockDim.y * blockIdx.y + threadIdx.y;
  const unsigned int tx = blockDim.x * blockIdx.x + threadIdx.x;

  // starting offset from higher dimensions
  for (int d = ndims - 1; d > 2; --d) {
  }

  // extent for 3D threadblock
  const int zext = (ndims >= 3) ? extents[2] : 1;
  const int yext = (ndims >= 2) ? extents[1] : 1;
  const int xext = (ndims >= 1) ? extents[0] : 1;

  for (int iz = tz; iz < zetx; iz += gridDim.z * blockDim.z) {
    for (int iy = ty; iy < yetx; iy += gridDim.y * blockDim.y) {
      for (int ix = tx; ix < xetx; ix += gridDim.x * blockDim.x) {
      }
    }
  }
...
void PackerStride2::pack(void *outbuf, int *position, const void *inbuf,
  ...
  assert(kernStream.size() > 0 &&
         "no streams. Are GPUs enabled and was MPI_Init called?");
